...
# miejsce na kod
dane %>%
  #Wyfiltruj filmy które jako Director maja Woody'ego Allena
  filter(Director == "Woody Allen") %>%
  #Stworz kolumne ktora przerzuci typ string Release.Date do typu Date w formacie dd/mm/yyyy, czyli %m/%d/%Y
  mutate(Release = (Release.Date %>% as.Date(format = '%m/%d/%Y'))) %>%
  #Posortuj rekordy wg kolumny Release (typu Date) w kolejnosci rosnacej
  arrange(Release) %>%
  #Wyciagnij kolumny Director, Title i Release
  select(Director, Title, Release) %>%
  #Wez pierwszy (najwczesniejsza data) rekord
  head(1)
```
...
dane %>%
  ...
  mutate(avg_score = mean(c(IMDb.Score * 10
           ...
           round(2)) %>%
  ungroup() %>%
  select(Director, Languages, Title, Release.Date, avg_score) %>%
...
# miejsce na kod
dane %>%
  #Utworz dwie kolumny Release i Release.Netflix które rzutuja typy daty ze string na Date
  mutate(Release = (Release.Date %>% as.Date(format = '%m/%d/%Y')),
         Release.Netflix  = (Netflix.Release.Date %>% as.Date(format = '%m/%d/%Y'))) %>%
  #Utworz zmienna Release.Year ktora mowi kiedy dana produkcja miala premiere
  mutate(Release.Year = format(Release, "%Y")) %>%
  #Wyfiltruj rekordy ktore nie zawieraja produkcji ktore wyszly w roku 2019 lub 2020
  filter(Release.Year == '2019' | Release.Year == '2020') %>% 
  #W tym bloku policz roznice miedzy kolumnami daty wyjscia produkcji na Netflix i wypuszczenia, przerzutuj to do typu DBL(liczba dni roznicy)
  rowwise() %>%
  mutate(netflix_release_delay = as.numeric(Release.Netflix - Release)) %>%
  ungroup() %>% 
  #Zwroc srednia dni miedzy wypuszczeiem produkcji a pojawieniem sie jej na netflixie
  summarise(Mean=mean(netflix_release_delay))
...
```{r 36, eval=FALSE}
# miejsce na kod
#Wyciagnij kolumnę tags jako wektor wartosci
tag_vector <- dane %>%
  filter(str_detect(Languages, 'Polish')) %>%
  select(Tags) %>%
  pull()
#W petli po kolei, kazdy wpis z kolumny tags rozdziel na wektor tagow 
tag_summary <- c()
for(tags_for_movie in tag_vector){
  tag_summary <- c(tag_summary, unlist(strsplit(tags_for_movie, ',')))
}

#Wyciagnij ilosc powtarzania sie danych tagow, posortuj od najwiekszego w dół
data.frame(tag_summary) %>% 
  count(tag_summary, sort=TRUE)
